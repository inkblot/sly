...
import static org.movealong.sly.app.Application.application;
...
class AppTest {
    ...
    @Test
    void runsAService() {
        ServiceHandle<Application<String, IO<?>, IO<String>>> handle = create();

        assertThat(run(bind(handle, service(io(application(() -> io("service")))))
    ...
    void detectsDuplicateBindings() {
        ...
        assertThrows(ServiceException.class,
                     () -> run(bind(key, service(io("first")))
                                   ...
                                   .andThen(bind(producerHandle, service(
                                       key, s -> service(io(application(() -> io("resolved: %" + s)))))))
                                   .andThen(resolve(producerHandle))));
    }
    ...
    void detectsResolutionCycles() {
        ...
        assertThrows(ServiceException.class,
                     () -> run(bind(aKey, service(bKey, b -> service(io("service A depends on " + b))))
                                   ...
                                   .andThen(bind(producerHandle, service(
                                       aKey, a -> service(io(application(() -> io("resolved: " + a)))))))
                                   .andThen(resolve(producerHandle))));
    }
