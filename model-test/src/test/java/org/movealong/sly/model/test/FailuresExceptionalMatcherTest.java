...
import static org.movealong.sly.model.test.FailuresExceptionalMatcher.isExceptionalThat;

class FailuresExceptionalMatcherTest {

    @Test
    void positiveMatch() {
        Failures          subject = exceptional(new RuntimeException("boom"));
        Matcher<Failures> matcher = isExceptionalThat(anything());
        assertTrue(matcher.matches(subject));
    }

    @Test
    void typeMismatches() {
        FailuresExceptionalMatcher matcher = isExceptionalThat(anything());
    ...
    @Test
    void exceptionMismatch() {
        Throwable                  exception   = new RuntimeException("boom");
        Failures                   exceptional = exceptional(exception);
        FailuresExceptionalMatcher matcher     = isExceptionalThat(hasMessage(containsString("bad")));
